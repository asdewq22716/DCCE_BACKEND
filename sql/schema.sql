...
COMMENT ON COLUMN organizations.id IS 'ID ลำดับที่ของหน่วยงาน';
...
COMMENT ON COLUMN users.id IS 'ID ลำดับที่ของผู้ใช้งานในระบบ';
...
COMMENT ON COLUMN permissions.p_group IS 'กลุ่มของสิทธิ์ (เช่น บัญชี, HR)';
