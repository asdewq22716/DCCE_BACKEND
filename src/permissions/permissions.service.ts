...
export class PermissionsService {
  ...
  async createPermission(dto: CreatePermissionDto) {
    try {
      ...
      throw new BadRequestException(
        'ไม่สามารถสร้างสิทธิ์ได้ (อาจมีรหัสสิทธิ์นี้อยู่แล้ว)',
      );
    }
  }
  ...
  async createGroup(dto: CreatePermissionGroupDto) {
    try {
      ...
      throw new BadRequestException(
        'ไม่สามารถสร้างกลุ่มสิทธิ์ได้ (ชื่อกลุ่มอาจซ้ำ)',
      );
    }
  }
  ...
  async deleteGroup(groupId: number) {
    ...
    const perms = await this.db.select('permissions', {
      group_id: groupId,
      is_active: 1,
    });
    if (perms.length > 0)
      throw new BadRequestException(
        'ไม่สามารถลบกลุ่มที่มีสิทธิ์การใช้งานอยู่ได้',
      );
