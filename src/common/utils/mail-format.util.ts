export class MailFormat {
  ...
  /**
   * แบบฟอร์มสำหรับติดต่อสอบถามจากบุคคลภายนอก (Public Contact)
   */
  static buildContactPublicHtml(title: string, message: string, senderName: string, senderEmail: string, phone: string): string {
    const timestamp = new Date().toLocaleString('th-TH', { timeZone: 'Asia/Bangkok' });
    ...
    return `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; border: 1px solid #e0e0e0; border-radius: 8px; overflow: hidden; box-shadow: 0 4px 6px rgba(0,0,0,0.05);">
        <!-- Header -->
        <div style="background-color: #2b6cb0; color: white; padding: 20px; text-align: center;">
          <h2 style="margin: 0; font-size: 24px;">✉️ ข้อความติดต่อสอบถามจากบุคคลภายนอก</h2>
        </div>
        ...
        <div style="padding: 24px; background-color: #ffffff;">
          ...
          <table style="width: 100%; border-collapse: collapse; margin-top: 20px; font-size: 14px;">
            ...
            <tr>
              <td style="padding: 10px 0; color: #777777; width: 140px;"><strong>เบอร์โทรศัพท์:</strong></td>
